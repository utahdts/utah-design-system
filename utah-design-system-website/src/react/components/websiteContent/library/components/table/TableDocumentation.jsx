  ...
  return (
    <div className="documentation-content">
      ...
      <StaticExample
        ...
        quickTips={(
          <ul>
            <li>Alternating row colors can help sighted users scan data in a table. Using only alternating row colors is not an acceptable solution
              for dividing rows since the contrast ratio is too low for those with low vision. You should combine alternating row background colors
              with borders to address minimum contrast ratios.
            </li>
          </ul>
        )}
      />
      ...
      <StaticExample
        ...
        quickTips={(
          <ul>
            <li>A table with column filters allows the user to quickly find information in the table. Consider using filters when the table data is
              larger than the viewport or over 10 rows long.
            </li>
          </ul>
        )}
      ...
      <ul className="mb-spacing">
        ...
        <li><strong>Displaying directories.</strong> When listing information, locations, or resources that have similarly structured content for many
          items.
        </li>
      </ul>
      ...
      <ul className="mb-spacing">
        ...
        <li><strong>Attribute table data in a caption.</strong> If your table includes information from a specific source or contains frequently
          updated content, provide the source and/or last updated date. This clarification is especially useful if your table summarizes data from a
          more extensive source.
        </li>
        <li><strong>Consider a small-screen experience.</strong> On mobile devices and other small screens, numerical data across many columns can be
          easier to understand if the table scrolls horizontally. Directory lists are more readable if the rows display in a stacked layout. For
          tables with more than two columns, make sure you choose either a scrollable or a stacked variant.
        </li>
        <li><strong>Minimize the number of columns.</strong> It&apos;s easier for users to read down a long list of rows than it is to read across a
          long list of columns. Eliminate columns when possible, or consider swapping the columns and rows to improve scannability.
        </li>
        <li><strong>Enable sort where useful.</strong> Add row sorting to individual columns of long tables where the data can be logically ordered
          either alphabetically or numerically.
        </li>
        <li><strong>Don&apos;t use row sorting with merged cells.</strong> Sorting won&apos;t work properly if your table contains colspan or rowspan
          attributes on the cells.
        </li>
        <li><strong>Don&apos;t use row sorting with the mobile stacked variants.</strong> Sorting won&apos;t work properly with these variants because
          the column headers at the top of the table don&apos;t appear at narrow widths and are instead moved into the cell content in each row.
        </li>
        <li><strong>Set a default sort column and direction.</strong> Set the default sort column and direction so that the table renders optimally on
          the first view.
        </li>
        <li><strong>Use table filters.</strong> Use table filters to allow users to quickly find information in tables that have large amounts of
          data. Match the table filter type with the information contained in the column. (For example: a column of data with a small data set of
          unique values could use a select list)
        </li>
      </ul>
      ...
      <ul className="mb-spacing">
        ...
        <li>The table&apos;s row boundaries must maintain a <code>3:1</code> contrast ratio against the background. (For example the border on top and
          bottom)
        </li>
        <li>When provided, the table&apos;s column boundaries should maintain a <code>3:1</code> contrast ratio. (For example the border on left and
          right)
        </li>
      </ul>
      <h4 id="section-contrast">Keyboard Interactivity</h4>
      <ul className="mb-spacing">
        <li><strong>Scrollable tables need to be focusable.</strong> When you make a table scrollable, you must add
          the <code>tabindex=&quot;0&quot;</code> attribute to the scrollable element. This attribute assures that users navigating with a keyboard
          are able to select and scroll the table. <code>tabindex=&quot;0&quot;</code> enables focus on elements that do not get focus by default.
          This attribute does not change the tab order. It places the element in the logical navigation flow.
        </li>
      </ul>
      ...
      <ul className="mb-spacing">
        ...
        <li><strong>Add an aria-live region to the page when enabling row sorting.</strong> An <code>aria-live</code> region immediately following
          the <code>&lt;table&gt;</code> element will automatically announces when the sort state changes for visitors using screen readers.
        </li>
        <li><strong>Use buttons for sorting.</strong> Use a button to allow the user to sort a column. This allows the button to receive focus and be
          pressed natively.
        </li>
        <li><strong>Aria-label attributes to sortable column headers.</strong> The <code>aria-label</code> attribute should be added to to the
        ...
        <li><strong>Complex tables have more than two levels of headers.</strong> Each header should have a unique <code>id</code> and each data cell
          should have a <code>headers</code> attribute with each related header cell&apos;s <code>id</code> listed.
        </li>
      ...
      <div className="documentation-content--small-text">
        <TabGroup defaultValue="table-props-css">
          <TabList>
            <Tab id="table-props-css">CSS</Tab>
            <Tab id="table-props-react">React</Tab>
          </TabList>
          <TabPanels>
            <TabPanel tabId="table-props-css">
              <TableCssClassesDocumentation />
            </TabPanel>

            <TabPanel tabId="table-props-react">
              <TablePropsDocumentation />
            </TabPanel>
          </TabPanels>
        </TabGroup>
      </div>
